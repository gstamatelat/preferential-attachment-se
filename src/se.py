...
def se_d(n: int, m: int, rng: Random, initial_graph: nx.Graph = None) -> nx.Graph:
    ...
    # Initialize the RSS data structure
    rss: RandomSystematicPartitioning = RandomSystematicPartitioning(m, rng).add_items(g.nodes, lambda x: g.degree[x])
    ...
    while source < n:
        ...
        if g.has_node(source):
            raise ValueError(f"The initial graph contains node {source} already")
        # Select one random old hyperedge.
        random_hyperedge: set[object] = rss.reshuffle().sample()
        # Insert its elements into the RSS. At the same time create the edges too.
        for v in random_hyperedge:
            rss.add_item(v, 1)
            g.add_edge(source, v)
        # Insert m copies of source into the RSS
        rss.add_item(source, m)
        # Advance source
        source += 1
